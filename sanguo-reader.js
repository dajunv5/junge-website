/* 三国演义阅读器 - 完整版（100回，滑动翻页，字体调节） */
(function(){
'use strict';

/* ========== 全局状态 ========== */
var currentChapterIdx = 0;
var fontSize = parseInt(localStorage.getItem('sg-font-size')) || 18;
var lineHeight = parseFloat(localStorage.getItem('sg-line-height')) || 2;
...
function parseURL() {
    ...
    if (isNaN(ch)) ch = 0;
    currentChapterIdx = Math.max(0, Math.min(ch, SANGUO_DATA.length - 1));
}

/* ========== 渲染章节 ========== */
function renderChapter() {
    var chapter = SANGUO_DATA[currentChapterIdx];
    if (!chapter) return;

    document.title = '第' + numToChinese(chapter.id) + '回 ' + chapter.title.replace(/^第[一二三四五六七八九十百零\d]+回[\s　]*/, '') + ' - 三国演义 - 有意思的书窗';

    // 导航标题
    var titleEl = document.getElementById('navTitle');
    if (titleEl) titleEl.textContent = '三国演义 · 第' + numToChinese(chapter.id) + '回';
...
function renderContent(chapter) {
    ...
    // 清理标题：去掉"第X回"前缀
    var displayTitle = chapter.title.replace(/^第[一二三四五六七八九十百零\d]+回[\s　]*/, '');

    var html = '';
    html += '<div class="chapter-header">';
    html += '<div class="chapter-idx">第' + numToChinese(chapter.id) + '回</div>';
    html += '<h1 class="chapter-title">' + escapeHtml(displayTitle) + '</h1>';
    html += '</div>';
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (!line) continue;
        // 跳过章节标题行（已在header中显示）
        if (line.indexOf('第') === 0 && line.indexOf('回') !== -1 && line.indexOf('回') < 12) {
            continue;
        }
...
function goToChapter(idx, direction) {
    if (idx < 0 || idx >= SANGUO_DATA.length) return;
...
function updateNavState() {
    ...
    if (nextBtn) nextBtn.disabled = (currentChapterIdx === SANGUO_DATA.length - 1);
    if (footerInfo) footerInfo.textContent = (currentChapterIdx + 1) + ' / ' + SANGUO_DATA.length;
    if (progress) {
        var pct = ((currentChapterIdx + 1) / SANGUO_DATA.length * 100).toFixed(1);
...
function onTouchMove(e) {
    ...
    if (swipeState.direction) {
        ...
        if (dx < 0 && currentChapterIdx === SANGUO_DATA.length - 1) dampDx = dx * 0.3;
...
function onTouchEnd(e) {
    ...
    if (shouldFlip) {
        ...
        } else if (dx < 0 && currentChapterIdx < SANGUO_DATA.length - 1) {
            // 向左滑 → 下一回
...
function showHint(dx) {
    ...
    if (dx < -30 && currentChapterIdx < SANGUO_DATA.length - 1 && hintRight) hintRight.classList.add('show');
}
...
function initChapterSelect() {
    ...
    if (!sel) return;
    sel.innerHTML = '';
    for (var i = 0; i < SANGUO_DATA.length; i++) {
        var ch = SANGUO_DATA[i];
...
function applyFontSize() {
    ...
    localStorage.setItem('sg-font-size', fontSize);
    localStorage.setItem('sg-line-height', lineHeight);
}
