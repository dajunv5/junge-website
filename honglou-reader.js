/* 红楼梦阅读器 - 完整版（100回，滑动翻页，字体调节） */
(function(){
'use strict';

/* ========== 全局状态 ========== */
var currentChapterIdx = 0;
var fontSize = parseInt(localStorage.getItem('hl-font-size')) || 18;
var lineHeight = parseFloat(localStorage.getItem('hl-line-height')) || 2;
...
function parseURL() {
    ...
    if (isNaN(ch)) ch = 0;
    currentChapterIdx = Math.max(0, Math.min(ch, HONGLOU_DATA.length - 1));
}

/* ========== 渲染章节 ========== */
function renderChapter() {
    var chapter = HONGLOU_DATA[currentChapterIdx];
    if (!chapter) return;

    document.title = '第' + numToChinese(chapter.id) + '回 ' + chapter.title.replace(/^第[一二三四五六七八九十百零\d]+回[\s　]*/, '') + ' - 红楼梦 - 有意思的书窗';

    // 导航标题
    var titleEl = document.getElementById('navTitle');
    if (titleEl) titleEl.textContent = '红楼梦 · 第' + numToChinese(chapter.id) + '回';
...
function goToChapter(idx, direction) {
    if (idx < 0 || idx >= HONGLOU_DATA.length) return;
...
function updateNavState() {
    ...
    if (nextBtn) nextBtn.disabled = (currentChapterIdx === HONGLOU_DATA.length - 1);
    if (footerInfo) footerInfo.textContent = (currentChapterIdx + 1) + ' / ' + HONGLOU_DATA.length;
    if (progress) {
        var pct = ((currentChapterIdx + 1) / HONGLOU_DATA.length * 100).toFixed(1);
...
function onTouchMove(e) {
    ...
    if (swipeState.direction) {
        ...
        if (dx < 0 && currentChapterIdx === HONGLOU_DATA.length - 1) dampDx = dx * 0.3;
...
function onTouchEnd(e) {
    ...
    if (shouldFlip) {
        ...
        } else if (dx < 0 && currentChapterIdx < HONGLOU_DATA.length - 1) {
            // 向左滑 → 下一回
...
function showHint(dx) {
    ...
    if (dx < -30 && currentChapterIdx < HONGLOU_DATA.length - 1 && hintRight) hintRight.classList.add('show');
}
...
function initChapterSelect() {
    ...
    if (!sel) return;
    sel.innerHTML = '';
    for (var i = 0; i < HONGLOU_DATA.length; i++) {
        var ch = HONGLOU_DATA[i];
...
function applyFontSize() {
    ...
    localStorage.setItem('hl-line-height', lineHeight);
}
