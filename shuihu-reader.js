/* 水浒传阅读器 - 完整版（100回，滑动翻页，字体调节） */
(function(){
'use strict';

/* ========== 全局状态 ========== */
var currentChapterIdx = 0;
var fontSize = parseInt(localStorage.getItem('sh-font-size')) || 18;
var lineHeight = parseFloat(localStorage.getItem('sh-line-height')) || 2;
...
function parseURL() {
    ...
    if (isNaN(ch)) ch = 0;
    currentChapterIdx = Math.max(0, Math.min(ch, SHUIHU_DATA.length - 1));
}
...
function renderChapter() {
    ...
    if (titleEl) titleEl.textContent = '水浒传 · 第' + numToChinese(chapter.id) + '回';
...
function goToChapter(idx, direction) {
    if (idx < 0 || idx >= SHUIHU_DATA.length) return;
...
function updateNavState() {
    ...
    if (nextBtn) nextBtn.disabled = (currentChapterIdx === SHUIHU_DATA.length - 1);
    if (footerInfo) footerInfo.textContent = (currentChapterIdx + 1) + ' / ' + SHUIHU_DATA.length;
    if (progress) {
        var pct = ((currentChapterIdx + 1) / SHUIHU_DATA.length * 100).toFixed(1);
...
function onTouchMove(e) {
    ...
    if (swipeState.direction) {
        ...
        if (dx < 0 && currentChapterIdx === SHUIHU_DATA.length - 1) dampDx = dx * 0.3;
...
function onTouchEnd(e) {
    ...
    if (shouldFlip) {
        ...
        } else if (dx < 0 && currentChapterIdx < SHUIHU_DATA.length - 1) {
            // 向左滑 → 下一回
...
function showHint(dx) {
    ...
    if (dx < -30 && currentChapterIdx < SHUIHU_DATA.length - 1 && hintRight) hintRight.classList.add('show');
}
...
function initChapterSelect() {
    ...
    if (!sel) return;
    sel.innerHTML = '';
    for (var i = 0; i < SHUIHU_DATA.length; i++) {
        var ch = SHUIHU_DATA[i];
...
function applyFontSize() {
    ...
    localStorage.setItem('sh-font-size', fontSize);
    localStorage.setItem('sh-line-height', lineHeight);
}
